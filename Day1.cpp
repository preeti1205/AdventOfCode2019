...
planets! To accurately calculate his position in space, safely align his warp drive, 
...
Any mass that would require negative fuel should instead be treated as if it requires zero fuel; the remaining mass, if any, is instead handled by wishing really hard, 
which has no mass and is outside the scope of this calculation.

So, for each module mass, calculate its fuel and add it to the total. 
Then, treat the fuel amount you just calculated as the input mass(3329926 - from part 1) and repeat the process, 
...
int FuelofFuel(long long int temp_sum){
	long long int sum = 0;
	while(temp_sum > 0){
		temp_sum = (temp_sum/3) - 2;
		sum += temp_sum > 0 ? temp_sum : 0;
	}
	return sum;
}
...
int main(){
	std::string line;
	std::ifstream infile;
	infile.open("textfiles/input_file1.txt");
	int sum = 0;
	while(getline( infile, line)){
		long long int num = std::stoi(line);
		sum += FuelofFuel(num);
	}
